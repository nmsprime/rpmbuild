...
idx=$(snmpwalk -v2c -c "$com" "$ip" .1.3.6.1.2.1.10.127.1.1.1.1.2 | sed -s '/INTEGER: 0$/d' | cut -d'=' -f1 | awk -F'.' '{print $NF}' | sed 's/ $//')
...
freq=$(snmpwalk -v2c -c "$com" "$ip" .1.3.6.1.2.1.10.127.1.1.1.1.2 | grep -f <(echo "$idx") | awk -F':' '{print $NF}' | sed 's/$/\/1000000/' | bc -l | sed 's/\.\?0\+$//')
