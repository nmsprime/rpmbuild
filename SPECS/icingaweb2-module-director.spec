...
UPDATE import_source_setting set setting_value = 'SELECT CONCAT(a.id, \'_\', a.name) AS id, a.name, IF(a.parent_id < 3 OR a.parent_id = a.id, \'\', b.id) AS parent, IF(a.community_ro <> \'\', a.community_ro, (SELECT ro_community FROM provbase WHERE deleted_at IS NULL)) AS ro_community, SUBSTRING_INDEX(a.ip, \':\', 1) AS ip, IF(INSTR(a.ip, \':\') > 0, SUBSTRING_INDEX(a.ip, \':\', -1), NULL) AS port , getTopParentNetelementType(a.netelementtype_id) as netelementtype_id, t.vendor FROM netelement AS a JOIN netelement AS b ON b.id = a.parent_id JOIN netelementtype AS t ON a.netelementtype_id = t.id WHERE a.netelementtype_id > 2 AND a.ip <> \'\' AND a.deleted_at IS NULL;' where source_id = 1 and setting_name = 'query';
REPLACE INTO sync_property VALUES (1,1,1,'generic-host-director','import',1,NULL,'override'),(2,1,1,'${ip}','address',2,NULL,'override'),(3,1,1,'${name}','display_name',3,NULL,'override'),(4,1,1,'${parent}','vars.parents',4,NULL,'override'),(5,1,1,'${netelementtype_id}','vars.netelementtype_id',5,NULL,'override'),(6,1,1,'${ro_community}','vars.ro_community',6,NULL,'override'),(7,1,1,'${netelementtype_id}','groups',7,NULL,'override'),(8,1,1,'${vendor}','vars.vendor',8,NULL,'override'),(9,1,1,'${port}','vars.port',9,NULL,'override');
EOF
...
REPLACE INTO import_source_setting VALUES (1,'query','SELECT a.id, a.name, IF(a.parent_id < 3 OR a.parent_id = a.id, \'\', b.id) AS parent, (SELECT ro_community FROM provbase WHERE deleted_at IS NULL) AS ro_community, a.ip, getTopParentNetelementType(a.netelementtype_id) as netelementtype_id FROM netelement AS a JOIN netelement AS b ON b.id = a.parent_id WHERE a.netelementtype_id > 2 AND a.ip <> \'\' AND a.deleted_at IS NULL;'),(1,'resource','nmsprime');
...
REPLACE INTO sync_property VALUES (1,1,1,'generic-host-director','import',1,NULL,'override'),(2,1,1,'${ip}','address',2,NULL,'override'),(3,1,1,'${name}','display_name',3,NULL,'override'),(4,1,1,'${parent}','vars.parents',4,NULL,'override'),(5,1,1,'${netelementtype_id}','vars.netelementtype_id',5,NULL,'override'),(6,1,1,'${ro_community}','vars.ro_community',6,NULL,'override'),(7,1,1,'${netelementtype_id}','groups',7,NULL,'override');
